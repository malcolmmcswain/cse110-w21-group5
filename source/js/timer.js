...
/**
 * Initializing tasklist in localStorage
 */
function initializeLocalStorage(){
    var emtptyJSON = "[]";
    localStorage.setItem("taskList", emtptyJSON);
}

/**
 * Get tasks (as JSON objects) in localStorage
 * @param {sting} task is the name of the task
 */
function getTaskObj(task){
    var taskList = JSON.parse(localStorage.getItem("taskList"));
    for (var i = 0; i < taskList.length; i++) {
        if (taskList[i].taskName == task){
            return taskList[i];
        }
    }
}
...
/** 
 * Update local storage to store tasks
 * @param {string} state needs to be either "complete" or "incomplete" 
 */ 
function updateTaskList(taskName, state){
    if(!JSON.parse(localStorage.getItem("taskList"))[0]){
        initializeLocalStorage();
    }
    var taskList = JSON.parse(localStorage.getItem("taskList"));
    for (var i = 0; i < taskList.length; i++) {
        // Search for the specified task in local storage,
        // Update if the task exists
    ...
    // Otherwise create a new task and add it to localStorage
    taskList.push({"taskName":taskName, "state":state})
    localStorage.setItem("taskList", JSON.stringify(taskList));
}

/** Remove specified task from the list
 * @param {string} task is the task to be removed
  */
...
/* Testing taskList stored in localStorage */
updateTaskList("meeting","incomplete");
updateTaskList("exam","complete");
console.log(getTaskObj('meeting'));
removeTask('meeting');
/* Remove when tasklist backend is verified to work correctly */
