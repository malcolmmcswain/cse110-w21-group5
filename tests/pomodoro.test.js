...
beforeEach(() => {
  ...
  <main>
    ...
    <div id="pomodoro-state">
      <text id="pomodoro-state-text">0&nbsp;&nbsp;|&nbsp;&nbsp;<image id="pomodoro-state-icon" src="./media/pomodoro.png"></image></text>
    </div>
  ...
  <div class="overlay" id="options-panel">
    <form id="options-form">
      ...
      <input type="number" id="long-length"/><br>
      <label for="cycle-length" id="cycle-length-label">Pomos per Cycle</label>
      <input type="number" id="cycle-length"/><br>
...
test('Status text to state conversion correct', () => {
  expect(pomodoro.convertStatusTextToState('Work')).toBe('work');
  expect(pomodoro.convertStatusTextToState('Short Break')).toBe('short_break');
  expect(pomodoro.convertStatusTextToState('Long Break')).toBe('long_break');
  expect(pomodoro.convertStatusTextToState('Stopped')).toBe('stopped');
  expect(pomodoro.convertStatusTextToState('Reset')).toBe('reset');
